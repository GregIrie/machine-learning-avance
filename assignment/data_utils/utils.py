...
def oov(X,out_of_vocab):
    if X not in out_of_vocab:
        return(0)
    else :
        return(1)
...
def get_oov(X_train,Y_train, X_test, Y_test):
    vocab_train=set(X_train.reshape(-1)) 
    vocab_test = set(X_test.reshape(-1))
    out_of_vocab = vocab_test - vocab_train
    vectorized_oov=vectorize(lambda x:oov(x,out_of_vocab)) 
    top_oov=vectorized_oov(X_test.reshape(-1))
    top_oov=top_oov.reshape(X_test.shape)
    top_oov=top_oov.sum(axis=1)
    df_oov=pd.DataFrame(top_oov,columns=['top'])
    X_test_oov = X_test[array(df_oov[df_oov.top==1].index)]
    Y_test_oov = Y_test[array(df_oov[df_oov.top==1].index)]
    return(X_test_oov,Y_test_oov)
